...
<script>
	window.onload =  function() {
		var replyPage = 1;
		getPage("<%=request.getContextPath()%>/reply/${board.bno}/"+replyPage)
...
//			alert("reply regist btn click")
			...
			$.ajax({
				...
				success : function(data) {
					alert("댓글이 등록되었습니다.")
					getPage("<%=request.getContextPath()%>/reply/${board.bno}/"+data)
					$("#newReplyText").val("");
				},
				error: function(e) {
					alert("댓글 등록에 실패했습니다.")
		...
		$("#replyModBtn").on("click", function(e) {
			...
			$.ajax({
				url : "<%=request.getContextPath()%>/reply/"+rno,
				type : "put",
				headers:{
		...
		$("#replyDelBtn").on("click", function(e) {
			...
			$.ajax({
				url : "<%=request.getContextPath()%>/reply/${board.bno}/"+rno+"/"+replyPage,
				type : "delete",
				headers:{
					"Content-Type" : "application/json",
				...
				success : function(page) {
					alert("삭제되었습니다.")
					replyPage = page;
					getPage("<%=request.getContextPath()%>/reply/${board.bno}/"+page)
				},
				error : function(error) {
					alert("삭제 실패했습니다.")
				},
